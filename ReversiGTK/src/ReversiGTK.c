...
GtkWidget *crear_tablero(){
	...
	for (i = 0; i < MAX_FILA; i++) {
		for (j = 0; j < MAX_COLUMNA; j++) {
			...
			if((i == 3 && j == 3) || (i == 4 && j == 4)){
				gtk_image_set_from_file(GTK_IMAGE(gtk_grid_get_child_at(GTK_GRID(tablero),j,i)), "IMG/sonic.png");
			}else if((i == 3 && j == 4) || (i == 4 && j == 3)){
				gtk_image_set_from_file(GTK_IMAGE(gtk_grid_get_child_at(GTK_GRID(tablero),j,i)), "IMG/eggman.png");
			}
